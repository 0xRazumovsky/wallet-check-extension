...
export function parseOpcodes(bytecode: string): string[] {
  ...
  const bytes: string[] = matches ? [...matches] : [];
  const opcodes: string[] = [];
  let i = 0;
  while (i < bytes.length) {
    const byte = bytes[i];
    if (!byte) break;
    const opcodeHex = parseInt(byte, 16);
    const opcodeName = OPCODES[opcodeHex] ?? "UNKNOWN";
    opcodes.push(opcodeName);
    if (opcodeName.startsWith("PUSH")) {
      const width = opcodeHex - 0x5f;
      i += width;
    }
    i += 1;
  }
