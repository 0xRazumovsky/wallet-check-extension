...
async function fetchSourcifyAbi(chainId, address) {
...
async function fetchEtherscanAbi(chainId, address) {
...
export async function getAbi(chainId, address) {
    if (!address)
        return null;
    ...
    if (sourcify) {
        const entry = { abi: sourcify, source: "sourcify" };
        abiCache.set(key, entry);
        return entry;
    }
    const etherscan = await fetchEtherscanAbi(chainId, address);
    if (etherscan) {
        const entry = { abi: etherscan, source: "etherscan" };
        abiCache.set(key, entry);
        return entry;
    }
    return null;
}
function formatParams(inputs, args) {
...
export function inferSignatureFromData(data) {
    if (!data || data === "0x" || data.length < 10)
        return undefined;
    const selector = data.slice(0, 10).toLowerCase();
    return knownSelectors[selector];
}
export function decodeCalldata(data, abi, fallbackSignature) {
    if (!data || data === "0x")
        return null;
    if (abi && abi.length > 0) {
        try {
